...
func TestAPIServerPSQL_CreateURL_and_GetFullURL(t *testing.T) {
	...
	for i, val := range Responses {
		...
		fmt.Println(colors.Cyan + "Get req: " + colors.Res + "\t" + cfg.Options.Schema + "//" + cfg.Options.Prefix + "/" + val.shortURL)
		res := httptest.NewRecorder()
